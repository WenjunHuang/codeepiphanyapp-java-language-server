...
import sun2.security.action.GetBooleanAction;
import sun2.security.action.GetIntegerAction;
import sun2.security.action.GetPropertyAction;
import sun2.security.provider.certpath.CertId;
import sun2.security.provider.certpath.OCSP;
import sun2.security.provider.certpath.OCSPResponse;
import sun2.security.provider.certpath.ResponderId;
import sun2.security.util.Cache;
import sun2.security.x509.PKIXExtensions;
import sun2.security.x509.SerialNumber;
import sun2.security.ssl.X509Authentication.X509Possession;
import static sun2.security.ssl.CertStatusExtension.*;
...
final class StatusResponseManager {
    ...
    StatusResponseManager() {
        int cap = AccessController.doPrivileged(
                new GetIntegerAction("jdk2.tls.stapling.cacheSize",
                    DEFAULT_CACHE_SIZE));
        cacheCapacity = cap > 0 ? cap : 0;

        int life = AccessController.doPrivileged(
                new GetIntegerAction("jdk2.tls.stapling.cacheLifetime",
                    DEFAULT_CACHE_LIFETIME));
        cacheLifetime = life > 0 ? life : 0;

        String uriStr = GetPropertyAction
                .privilegedGetProperty("jdk2.tls.stapling.responderURI");
        URI tmpURI;
        try {
        ...
        respOverride = AccessController.doPrivileged(
                new GetBooleanAction("jdk2.tls.stapling.responderOverride"));
        ignoreExtensions = AccessController.doPrivileged(
                new GetBooleanAction("jdk2.tls.stapling.ignoreExtensions"));
    ...
    class OCSPFetchCall implements Callable<StatusInfo> {
        ...
        public StatusInfo call() {
            ...
            try {
                ...
                // conditions occur:
                // (1) The jdk2.tls.stapling.ignoreExtensions property is
                //     true, or
