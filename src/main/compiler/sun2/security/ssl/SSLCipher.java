...
import sun2.security.ssl.Authenticator.MAC;
import static sun2.security.ssl.CipherType.*;
import static sun2.security.ssl.JsseJce.*;
...
enum SSLCipher {
    ...
    // Map of Ciphers listed in jdk2.tls.keyLimits
    private static final HashMap<String, Long> cipherLimits = new HashMap<>();

    // Keywords found on the jdk2.tls.keyLimits security property.
    ...
    static  {
        ...
        String prop = AccessController.doPrivileged(
            ...
            @Override
            public String run() {
                return Security.getProperty("jdk2.tls.keyLimits");
            }
        });
            ...
            for (String entry : propvalue) {
                ...
                if (values[1].contains(tag[0])) {
                    ...
                    if (SSLLogger.isOn && SSLLogger.isOn("ssl")) {
                        SSLLogger.fine("jdk2.tls.keyLimits:  Unknown action:  " +
                                entry);
                    }
                    continue;
                }
                ...
                try {
                    ...
                    if (SSLLogger.isOn && SSLLogger.isOn("ssl")) {
                        SSLLogger.fine("jdk2.tls.keyLimits:  " + e.getMessage() +
                                ":  " +  entry);
                    }
                    continue;
                }
                if (SSLLogger.isOn && SSLLogger.isOn("ssl")) {
                    SSLLogger.fine("jdk2.tls.keyLimits:  entry = " + entry +
